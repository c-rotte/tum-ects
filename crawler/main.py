    ...
    def run(self):
        ...
        try:
            for degree_id, mapping_info in self.crawler.module_degree_mappings(self.module_id):
                degree_exists = False  # Degree.select().where(Degree.degree_id == degree_id).exists()
                module_exists = False  # Module.select().where(
                # Module.module_id == self.module_id).exists()
